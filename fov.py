...
#----------------------------------------------------------------------------------------#
#------------prepare the timescale and location------------------------------------------#
#----------------------------------------------------------------------------------------#
...
kaifeng = earth + Topos(longitude_degrees=(114,30,0),
                        latitude_degrees=(+34,8,0)) # 34

#----------------------------------------------------------------------------------------#
#-------------prepare the fig and ax-----------------------------------------------------#
#----------------------------------------------------------------------------------------#
fig = plt.figure()
...
# turn the horisontal circle to radec

fov_az = np.arange(0,360+0.1,1)
fov_alt = np.zeros(len(fov_az))

def get_radec(local,t,alt,az):
    a = [local.at(t).from_altaz(alt_degrees=i,az_degrees=j) for i,j in
            zip(alt,az)]
    ras = [np.radians(i.radec()[0]._degrees) for i in a]
    decs = [45-0.5*i.radec()[1].degrees for i in a]

    return ras,decs

fov_ras, fov_decs = get_radec(kaifeng,t,fov_alt,fov_az)

ax.plot(fov_ras,fov_decs,color='yellow',linewidth=2)

# plot the N, W ,E ,S ,four direction marks

dir_alt = [0,0,0,0]
dir_az = [0,90,180,270]
dir_txt =['N','E','S','W']
dir_ras, dir_decs  = get_radec(kaifeng,t,dir_alt,dir_az)

# rotate the map
# according to West as 0
ax.set_theta_offset(dir_ras[-1])

# set brim
# according to max of South

brim = max(dir_decs) + 2 # 2 -- margin
ax.set_ylim(0, brim)

ax.scatter(dir_ras,dir_decs,color='red',s=10)

#----------------------------------------------------------------------------------------#
#-----------plot ecliptic , which is  the sun orbit--------------------------------------#
#----------------------------------------------------------------------------------------#
...
t_oneyear = [ts.utc(i.year, i.month, i.day, i.hour, i.minute, i.second)
        for i in d_oneyear]
...
decs_sun = [45 -i/2 for i in decs_sun] # linear trans

## get moon ra dec in 30 days

d_onemonth = [d1 +dt.timedelta(hours=1)*i for i in range(24*30+1)]
t_onemonth = [ts.utc(i.year, i.month, i.day, i.hour, i.minute, i.second)
        for i in d_onemonth]
ras_moon = []
decs_moon = []
...
decs_moon = [45 -i/2 for i in decs_moon] # linear trans
